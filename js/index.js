...
function renderWhiteSauce() {
 // Iteration 2: add/remove the class "sauce-white" of `<section class="sauce">`
    document.querySelectorAll('.sauce').forEach((saucee) => {
      if(state.whiteSauce){
        saucee.classList.add('sauce-white')
      }else{
        saucee.classList.remove('sauce-white');
      }
    })
...
function renderPrice() {
  ...
  let startingPizza = document.querySelector(".panel.price ul");
  startingPizza.innerHTML = " ";
  ...
  for (let ingr in state){
    if(state[ingr]){
      ...
      listIngre.textContent = `$${ingredients[ingr].price} ${ingredients[ingr].name}`
      startingPizza.appendChild(listIngre)

      totalPrice += ingredients[ingr].price
    }
  }
  const pizzaPrice = document.querySelector(".panel.price strong");
  pizzaPrice.textContent = `$${totalPrice}`
